                        ...
                        onSearch={(query) => {
                            ...
                            setFilteredPermissions(
                                availablePermissions.filter(permission =>
                                    permission.perm_name.toLowerCase().includes(lowerQuery) ||
                                    ...
                                    <FormItem className="flex items-center gap-2">
                                        <FormControl>
                                            <Checkbox disabled={isDisabled} className="h-5 w-5 border-gray-300"
                                                checked={field.value?.includes(permission.perm_id)}
                                                onCheckedChange={(checked) => {
                                                    const newValue = checked
                                                        ? [...(field.value ?? []), permission.perm_id]
                                                        : field.value?.filter((id) => id !== permission.perm_id)
                                                    field.onChange(newValue)
                                                }}
                                            />
                                        </FormControl>
                                        <div>
                                            <FormLabel className="line-clamp-1 max-w-[180px]  break-all overflow-hidden">{permission.perm_name}</FormLabel>
                                            <FormDescription className=" line-clamp-1 break-all max-w-[220px] overflow-hidden">{permission.description}</FormDescription>
                                        </div>
                                        <FormMessage />
                                    </FormItem>
                                )}
